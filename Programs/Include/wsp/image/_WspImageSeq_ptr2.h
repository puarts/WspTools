...
public:
    ...
    inline wsp::State SetSeqSize(int num_of_seq){ 
        ...
        wsp::State state;
        int w, h, nc;
        int old_len = data_->length();
        if(old_len!=0){
            w = width(), h = height(), nc = num_of_channels();
        }else{
            w = 1, h = 1, nc = 1;
        }
        state = data_->SetLength(num_of_seq);
        if(state!=WSP_STATE_SUCCESS){ return state; }
        int new_len = data_->length();
        for(int i=old_len; i<new_len; ++i){
            (*data_)[i] = WSP_NEW wsp::Image<T>(w, h, nc);
        }
        return WSP_STATE_SUCCESS;
    }
...
template<class T> wsp::Image<T>* wsp::ImageSeq<T>::operator[](u32 index){ 
    if(data_==NULL){
        //data_ = new wsp::Vector<wsp::Image<T>*>(index+1);
        return NULL;
    }
    if(data_->length()<=index){
        WSP_COMMON_WARNING_LOG("index is smaller than sequence size\n"); return NULL; 
    }
    return (*data_)[index]; 
}
