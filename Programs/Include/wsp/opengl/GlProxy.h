...
namespace wsp {
    namespace opengl {
        namespace glproxy {
            ...
            //! @brief この関数でセットされたコマンドキューに対して各コマンドを積みます。
            //! nullptr の場合は各コマンドは即実行されます。
            WSP_DLL_EXPORT void SetCurrentCommandQueue(GpuCommandQueue* command_queue);

            WSP_DLL_EXPORT GpuCommandQueue* GetCurrentCommandQueue();

            //! @brief 現在のコマンドキューのコマンドをすべて実行します。
            WSP_DLL_EXPORT void ExecuteCommands();
            ...
            WSP_DLL_EXPORT void UniformMatrix3f(GLint location,
                GLboolean transpose,
                const GLfloat *value);
            ...
            WSP_DLL_EXPORT void UniformMatrix4f(GLint location,
                GLboolean transpose,
                const GLfloat *value);
            ...
            WSP_DLL_EXPORT void ClientWaitSync(GLsync sync,
                GLbitfield flags,
                GLuint64 timeout);
