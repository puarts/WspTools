...
#ifndef __WSP_CORE_TEMPLATE_CLASS_VECTOR_INL__
#define __WSP_CORE_TEMPLATE_CLASS_VECTOR_INL__
...
wsp::Vector<T, MemAllocStep>::Vector()
    : data_(NULL),
    length_(0),
    num_of_channels_(1),
    num_of_elem_(0),
...
{}
template<class T, int MemAllocStep>
wsp::Vector<T, MemAllocStep>::Vector(int length, int num_of_channels)
    : Vector()
{
    ...
    //    //data_ = WSP_NEW T[length_*num_of_channels_+RESERVE_MEM];
...
template<class T, int MemAllocStep>
wsp::Vector<T, MemAllocStep>::Vector(const T* ary, int length, int num_of_channels)
    : Vector()
{
...
template<class T, int MemAllocStep>
wsp::Vector<T, MemAllocStep>::Vector(const wsp::Vector<T, MemAllocStep>& vec)
    : Vector()
{
    *this = vec;
}
...
wsp::Vector<T, MemAllocStep>::~Vector()
{
    //printf("wsp::Vector Destructor\n");
    if(has_own_data_){
        //delete[] data_;
        //free(data_);
        wsp::FreeArrayMem(&data_);
        //printf("wsp::Vector destroyed\n");
    }
}
...
// Accessors: Getter --------------------------------------------------------------------------
template<class T, int MemAllocStep> 
inline int wsp::Vector<T, MemAllocStep>::length() const
{ 
    return length_; 
}

template<class T, int MemAllocStep> 
inline int wsp::Vector<T, MemAllocStep>::num_of_channels() const
{ 
    return num_of_channels_; 
}

template<class T, int MemAllocStep> 
inline T* wsp::Vector<T, MemAllocStep>::data()
{ 
    return data_; 
}

template<class T, int MemAllocStep> 
inline const T* wsp::Vector<T, MemAllocStep>::data() const
{ 
    return data_; 
}

template<class T, int MemAllocStep> 
inline T wsp::Vector<T, MemAllocStep>::GetElem(int index) const
{
...
//template<class T, int MemAllocStep> inline T wsp::Vector<T, MemAllocStep>::GetElem(int index) const{
//    if(index < 0){ return T(0); }
//    return GetElem(static_cast<int>(index));
//}
template<class T, int MemAllocStep> 
inline T wsp::Vector<T, MemAllocStep>::GetMax(int channel) const
{
...
template<class T, int MemAllocStep> inline T wsp::Vector<T, MemAllocStep>::GetMin(int channel) const{
    if(length_==0 || data_==NULL){ return (T)0; }
    T min = data_[0];
    T *ptr, *end_ptr;
    int len = length_*num_of_channels_;
...
template<class T, int MemAllocStep> inline T wsp::Vector<T, MemAllocStep>::GetSum(int channel) const{
    if(length_==0 || data_==NULL){ return (T)0; }
    T sum=data_[0];
    T *ptr, *end_ptr;
    int len = length_*num_of_channels_;
...
template<class T, int MemAllocStep> inline T wsp::Vector<T, MemAllocStep>::GetAverage(int channel) const{
    if(num_of_elem_==0){ 
...
template<class T, int MemAllocStep> inline bool wsp::Vector<T, MemAllocStep>::HasElem(T elem) const{
    int len = length_*num_of_channels_;
    for(int i=0; i<len; i++){
...
template<class T, int MemAllocStep> inline int wsp::Vector<T, MemAllocStep>::GetIndex(T elem) const{
...
template<class T, int MemAllocStep> 
void wsp::Vector<T, MemAllocStep>::SetDataFrom(T* data, int length, int num_of_channels){
    Clear();
    //wsp::FreeArrayMem(&data_);
    //free(data_);
    //delete[] data_;
...
template<class T, int MemAllocStep> 
void wsp::Vector<T, MemAllocStep>::SetDataInto(T** o_data, int *o_length, int *o_num_of_channels){
    *o_data = data_;
...
wsp::State wsp::Vector<T, MemAllocStep>::CopyFrom(const U* ary, int length, int num_of_channels, int skipStep){
    //wsp::FreeArrayMem(&data_);
    //free(data_);
    //delete[] data_;
    if(ary==NULL || length==0){
...
template<class T, int MemAllocStep> void wsp::Vector<T, MemAllocStep>::SetElem(const T& element, int index, int channel){
...
template<class T, int MemAllocStep> 
wsp::State wsp::Vector<T, MemAllocStep>::SetLength(int length, int num_of_channels)
{
    ...
    //wsp::Vector<T, MemAllocStep> tmp(*this);
...
template<class T, int MemAllocStep> 
void wsp::Vector<T, MemAllocStep>::Append(const T& element, int length){
    if(has_own_data_==false)
    { 
        WSP_COMMON_ERROR_LOG("You can not reallocate image data because current data is external data\n"); 
        return; 
...
template<class T, int MemAllocStep> 
T wsp::Vector<T, MemAllocStep>::Pop()
{
...
template<class T, int MemAllocStep> 
wsp::State wsp::Vector<T, MemAllocStep>::Reset(int startIndex, int endIndex, const T& value)
{
...
template<class T, int MemAllocStep> 
wsp::State wsp::Vector<T, MemAllocStep>::Reset(const T& value){
    return Reset(0, length_-1, value);
}

template<class T, int MemAllocStep> 
void wsp::Vector<T, MemAllocStep>::ResetElems(const T *src_elem, size_t elem_size)
{
    {
        T *ptr = data_;
...
template<class T, int MemAllocStep> inline wsp::State wsp::Vector<T, MemAllocStep>::Clear(){
    if(has_own_data_==false)
    { WSP_COMMON_ERROR_LOG("You can not reallocate image data because current data is external data\n"); return WSP_STATE_FAILURE; }
    wsp::FreeArrayMem(&data_);
    InitMemberVariables();
...
template<class T, int MemAllocStep> void wsp::Vector<T, MemAllocStep>::SortAscending(){
    qsort(data_, num_of_elem_, sizeof(T), wsp::CompA<T>);
}
template<class T, int MemAllocStep> void wsp::Vector<T, MemAllocStep>::SortDescending(){
    qsort(data_, num_of_elem_, sizeof(T), wsp::CompB<T>);
}
template<class T, int MemAllocStep> void wsp::Vector<T, MemAllocStep>::Reverse()
{
...
template<class T, int MemAllocStep> void wsp::Vector<T, MemAllocStep>::ShiftElements(double shiftSubpix)
{
...
template<class T, int MemAllocStep> void wsp::Vector<T, MemAllocStep>::SetRandNoise(double min, double max){
...
template<class T, int MemAllocStep> void wsp::Vector<T, MemAllocStep>::Abs(){
    int len = length_*num_of_channels_;
    for(int i=0; i<len; i++)
    {
...
template<class T, int MemAllocStep> void wsp::Vector<T, MemAllocStep>::Normalize(T min_norm, T max_norm){
...
template<class T, int MemAllocStep> void wsp::Vector<T, MemAllocStep>::CombineChannelsByFunc(T(*pre_func)(T), T(*post_func)(T)){
    if(num_of_channels_<=1){ return; }
    int i, c;
    double sum;
    wsp::Vector<T, MemAllocStep> tmp_vec(length_, 1);
    for(i=0; i<length_; ++i){
        sum = 0;
        for(c=0; c<num_of_channels_; c++){
...
template<class T, int MemAllocStep> void wsp::Vector<T, MemAllocStep>::CombineChannelsByNorm()
{
    return wsp::Vector<T, MemAllocStep>::CombineChannelsByFunc(wsp::SquareOperation, wsp::SquareRootOperation);
}
template<class T, int MemAllocStep> void wsp::Vector<T, MemAllocStep>::CombineChannelsBySum()
{
    return wsp::Vector<T, MemAllocStep>::CombineChannelsByFunc(wsp::NoOperation);
}
template<typename T, int MemAllocStep> 
wsp::State wsp::Vector<T, MemAllocStep>::SetNumOfChannels(int num_of_channels)
{
    if( num_of_channels == num_of_channels_ )
    {
        return WSP_STATE_SUCCESS;
    }
    wsp::Vector<T, MemAllocStep> tmp_vec(*this);
...
template<typename T, int MemAllocStep> 
wsp::State wsp::Vector<T, MemAllocStep>::ExtendsNumOfChannels( int extended_num_of_channels )
{
...
template<typename T, int MemAllocStep> 
wsp::State wsp::Vector<T, MemAllocStep>::LeaveOneChannel(int channel_index) 
{
    WSP_REQUIRES(
...
template<typename T, int MemAllocStep> 
wsp::State wsp::Vector<T, MemAllocStep>::RemoveExceptOneChannel(int channel_index )
{
    for(int i=0; i<length_; ++i)
    {
...
template<typename T, int MemAllocStep> 
s32 wsp::Vector<T, MemAllocStep>::TakeAllMatchedElems( T target_elem )
{
    T* ptr     = data_;
...
template<typename T, int MemAllocStep> 
s32 wsp::Vector<T, MemAllocStep>::TakeFirstMatchedElem( T target_elem )
{
    T* ptr     = data_;
    T* end_ptr = ptr + length_;
    T store_elem = *ptr;
...
template<typename T, int MemAllocStep> 
wsp::Result wsp::Vector<T, MemAllocStep>::InsertElem( s32 index, T elem )
{
...
// I/O ---------------------------------------------------------------------------------------
template<class T, int MemAllocStep> wsp::State wsp::Vector<T, MemAllocStep>::SaveRawData(const char *in_fileNameFormat,...) const{
...
template<class T, int MemAllocStep> wsp::State wsp::Vector<T, MemAllocStep>::LoadRawData(const char *fileName){
    if(has_own_data_==false)
    { 
        WSP_COMMON_ERROR_LOG("You can not reallocate image data because current data is external data\n"); 
        return WSP_STATE_FAILURE; 
...
//
//#ifdef USE_QT
//template<class T, int MemAllocStep> inline wsp::State wsp::Vector<T, MemAllocStep>::SaveRawDataDialog(const char *title) const{
...
//}
//
//template<class T, int MemAllocStep> inline wsp::State wsp::Vector<T, MemAllocStep>::LoadRawDataDialog(const char *title){
...
// Operators ---------------------------------------------------------------------------------
template<class T, int MemAllocStep> inline wsp::Vector<T, MemAllocStep>& wsp::Vector<T, MemAllocStep>::operator=(const wsp::Vector<T, MemAllocStep>& other){
    if(has_own_data_==false)
    { 
        WSP_COMMON_ERROR_LOG("You can not reallocate image data because current data is external data\n"); 
        return *this; 
...
template<class T, int MemAllocStep> inline bool wsp::Vector<T, MemAllocStep>::operator==(const wsp::Vector<T, MemAllocStep> &other) const{
...
template<class T, int MemAllocStep> 
inline bool wsp::Vector<T, MemAllocStep>::operator!=(const wsp::Vector<T, MemAllocStep> &other) const
{
    return !(*this==other);
}

template<class T, int MemAllocStep> 
inline T& wsp::Vector<T, MemAllocStep>::operator[](int index)
{ 
    return data_[index]; 
}
//template<class T, int MemAllocStep> inline T& wsp::Vector<T, MemAllocStep>::operator[](int index){
//    if(index<0){ return data_[0]; }
//    return operator[](static_cast<int>(index));
//}
template<class T, int MemAllocStep> 
inline const T& wsp::Vector<T, MemAllocStep>::operator[](int index) const
{
    WSP_ASSERT(0 <= index && index < num_of_elem_, "index exceeds size of vector, %d>=%d\n", index, num_of_elem_);
    return data_[index];
}
//template<class T, int MemAllocStep> inline const T wsp::Vector<T, MemAllocStep>::operator[](int index) const{
//    if(index<0){ return data_[0]; }
//    return operator[](static_cast<int>(index));
//}
template<class T, int MemAllocStep> 
inline wsp::Vector<T, MemAllocStep>& wsp::Vector<T, MemAllocStep>::operator+=(const T& right){
...
template<class T, int MemAllocStep> 
inline wsp::Vector<T, MemAllocStep>& wsp::Vector<T, MemAllocStep>::operator+=(const wsp::Vector<T, MemAllocStep>& vec){
...
template<class T, int MemAllocStep> 
inline wsp::Vector<T, MemAllocStep> wsp::Vector<T, MemAllocStep>::operator+(const T& right) const
{
...
template<class T, int MemAllocStep> 
inline wsp::Vector<T, MemAllocStep> wsp::Vector<T, MemAllocStep>::operator+(const wsp::Vector<T, MemAllocStep>& vec) const{
...
template<class T, int MemAllocStep> wsp::Vector<T, MemAllocStep> wsp::Vector<T, MemAllocStep>::operation(const T& value, T(*op)(const T& left, const T& right)) const{
    wsp::Vector<T, MemAllocStep> vec(length_);
    int len = length_*num_of_channels_;
    T *data_ptr = vec.data();
...
template<class T, int MemAllocStep> wsp::Vector<T, MemAllocStep> wsp::Vector<T, MemAllocStep>::operation(const wsp::Vector<T, MemAllocStep>& vec, T(*op)(const T& left, const T& right)) const{
    wsp::Vector<T, MemAllocStep> vec_dst(*this);
...
template<class T, int MemAllocStep> wsp::Vector<T, MemAllocStep> wsp::Vector<T, MemAllocStep>::division(const T& value) const{
    wsp::Vector<T, MemAllocStep> data(*this);
...
template<class T, int MemAllocStep> 
wsp::Vector<T, MemAllocStep> wsp::Vector<T, MemAllocStep>::division(const wsp::Vector<T, MemAllocStep>& vec) const{
    wsp::Vector<T, MemAllocStep> data(*this);
    int len_s = vec.length();
    if(vec.data()==NULL || len_s==0){
        return data;
...
template<class T, int MemAllocStep> 
std::ostream& operator<<(std::ostream& lhs, const wsp::Vector<T, MemAllocStep>& rhs)
{
